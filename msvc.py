...
import idc
from idaapi import BADADDR, FF_DATA, FF_DWRD, FF_0OFF, get_struc_size, FF_ASCI, \
                   do_unknown_range, DOUNK_DELNAMES, doStruct, get_member_by_name, \
                   get_32bit, get_ascii_contents, demangle_name, doDwrd, op_offset
...
class RTTIStruc:
    ...
    def __init__(self, tid, struc, size):
        self.tid = tid
        self.struc = struc
        self.size = size
...
def strip(name):
    ...
    if name.startswith("struct ") and name.endswith("`RTTI Type Descriptor'"):
        return name[7:-23]
...
class RTTICompleteObjectLocator(RTTIStruc):
    ...
    msid = add_struc(BADADDR, "RTTICompleteObjectLocator", False)
    set_struc_cmt(msid, "RTTI Complete Object Locator (#SusanRTTI)", True)
    ...
    def __init__(self, ea, vtable):
        ...
        if doStruct(ea, self.size, self.tid):
            ...
            if rtd.class_name:
                ...
                idc.MakeUnknown(ea, self.size, idc.DOUNK_SIMPLE)
...
class RTTITypeDescriptor(RTTIStruc):
    ...
    if msid != BADADDR:
        del_struc(get_struc(msid))
    msid = add_struc(BADADDR, "RTTITypeDescriptor", False)
    set_struc_cmt(msid, "RTTI Type Descriptor (#SusanRTTI)", True)
...
class RTTIClassHierarchyDescriptor(RTTIStruc):
    ...
    if msid != BADADDR:
        del_struc(get_struc(msid))
    msid = add_struc(BADADDR, "RTTIClassHierarchyDescriptor", False)
    set_struc_cmt(msid, "RTTI Class Hierarchy Descriptor (#SusanRTTI)", True)
    ...
    def __init__(self, ea):
        ...
        if doStruct(ea, get_struc_size(self.tid), self.tid):
            ...
            for i in range(1, nb_classes):
                ...
                op_offset(baseClasses+i * 4, -1, u.REF_OFF | idc.REFINFO_RVA, -1, 0, 0)
...
class RTTIBaseClassDescriptor(RTTIStruc):
    ...
    if msid != BADADDR:
        del_struc(get_struc(msid))
    msid = add_struc(BADADDR, "RTTIBaseClassDescriptor", False)
    set_struc_cmt(msid, "RTTI Base Class Descriptor (#SusanRTTI)", True)
    add_struc_member(get_struc(msid), "pTypeDescriptor", BADADDR, FF_DATA | FF_DWRD | FF_0OFF, u.mt_rva(), 4)
